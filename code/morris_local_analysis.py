...
def define_problem(input_data):
    ...
    for item in filtered_data:
        name = f"{item['activity']}" #f"{item['activity']}, {item['exchange']}"
...
def perform_local_morris_analysis(fu, 
 ...
 #       else:
    ...
    unique_groups = set(problem['groups'])
    updated_problem = {}
    ...
    #print(morris_samples['Electricity, gas, steam and air conditioning supply'])
    ...
    for category, samples in morris_samples.items():
        ...
        for alr_values in samples:
            ...
            compositions = []
            for group in unique_groups:
                inverse = ilr_inverse(grouped_data[group])
                #print(inverse) 
                compositions.append(inverse)
    ...
    print(X_dict['Electricity, gas, steam and air conditioning supply'])
    ...
    for index, (category, sample) in enumerate(X_dict.items()):
        ...
        filtered_exchanges = [
            exc for exc in exchanges 
            if exc['category'] in groupings[index]
        ]
       # print(f"{len(filtered_exchanges)} filtered vs {len(sample)} X_dict")
    ...
    #print(results['Civil engineering'])
    for entry in results['Electricity, gas, steam and air conditioning supply']:
    ...
    #morris_results.to_csv(f"results//Local_{market_dir}_results.csv", index=False)
    ...
    for market in SA_results.keys():
        ...
        if market in names_list:
            ...
            data.append({'names': market, 'mu_star': mu_star_value})
    ...
    #df_filtered.to_csv(f"results/Local_{market_dir}_scores.csv", index=False)
