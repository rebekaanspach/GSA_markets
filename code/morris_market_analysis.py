...
def perform_morris_market_analysis(fu, 
                                 amount,
                                 reference_product,
                                 d_label,  
                                 methods, 
        ...
        for exc in group:
            ...
            if (
                ...
                #    ) or
                 ...
                 ("biowaste" in exchange_text  and "biowaste" not in activity_text) or
 ...
 #       else:
    ...
    problem["names"] = [name if group in category_names else 'other' 
                    for group, name in zip(problem['groups'], problem['names'])]

# exchanges = [
#  exc for exc in exchanges if str(exc['category'])[:2] in category_numbers
# ]
...
#   df = df[df['groups'].isin(category_names)]
    ...
    group_masks = {group: (groups == group) for group in unique_groups}

    # Generator to avoid storing all intermediate results
    def generate_compositions():
        for alr_values in morris_samples:
            grouped_data = {group: alr_values[mask] for group, mask in group_masks.items()}
            compositions = (ilr_inverse(grouped_data[group]) for group in unique_groups)
            yield np.hstack(compositions)

    # Construct X with list comprehension and convert to array with explicit dtype
    X = np.array(list(generate_compositions()), dtype=np.float32).T

    # Add zeros
    zeros_block = np.zeros((X.shape[0], 3), dtype=np.float32)
    X_with_zeros = np.hstack((zeros_block, X))

    # Convert to NumPy array early for better memory access
    X_with_zeros = np.asarray(X_with_zeros, dtype=np.float32)

    # Precompute negative mask
    was_negative = np.array([exc['value'] < 0 for exc in exchanges])
    print(len(exchanges))

    # Apply sign flipping using NumPy broadcasting
    samples_with_negatives = np.where(was_negative[:, None], -X_with_zeros, X_with_zeros)
    print(len(samples_with_negatives))
   ...
   # X = np.array(X)
   # X = X.T
   # X_with_zeros = np.hstack([np.zeros((X.shape[0], 3), dtype=np.float32), X])
   ...
   # morris_results.to_csv(f"results//Global_{market_dir}_market_results.csv", index=False)
...
# count = count.reset_index(drop=True)
 #   SA = pd.concat([SA, count], axis=1)
    SA.to_csv(f"results/Global_{market_dir}_market_scores_level_{num_levels}_{trajectories}_trajectory.csv", index=False)
